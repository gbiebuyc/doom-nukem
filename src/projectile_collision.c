...
/*   Updated: 2019/06/26 21:19:44 by gbiebuyc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (tmp)
	{
		if (tmp->type == IS_MONSTER && d->monsters[tmp->id].can_collide)
		{
			dist = vec3f_length(sub_vec3f(newpos,
				...
				(sector->floorheight + d->monster_type[d->monsters[tmp->
				id].id_type].floating) + d->monster_type[d->monsters[tmp->
				id].id_type].height * 0.5, d->monsters[tmp->id].pos.y}));
				...
				d->monsters[tmp->id].size)
			{
				...
				vec2f_tmp[1] = (t_vec2f){d->monster_type[d->monsters[tmp->
					id].id_type].hitbox_radius + d->projectile_type[projectile->
					id_type].hitbox_radius, 0.0};
				actualize_dir(atan2(vec2f_tmp[0].y,
							vec2f_tmp[0].x), &vec2f_tmp[1]);
				...
				monster_hit(d, d->projectile_type[projectile->
						id_type].damage, tmp->id);
				return (true);
			}
		}
		tmp = tmp->next;
