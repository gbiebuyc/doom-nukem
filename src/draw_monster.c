...
void	display_sprite(t_display_data a, t_data *d, SDL_Surface *s, bool rev)
{
	int			x;
	int			y;
	int			colo;
	...
	while (x <= a.cut_end)
	{
		y = ft_max(a.ytop[x], a.start.y);
		while (y <= ft_min(a.ybot[x], a.end.y))
		{
			if (rev)
				colo = getpixel(s, (1 - a.scale.x * (x - a.start.x)),
				a.scale.y * (y - a.start.y));
			else
				colo = getpixel(s, a.scale.x * (x - a.start.x),
						a.scale.y * (y - a.start.y));
			{
...
void	draw_monster(t_data *d, t_sector *sector,
		t_frustum *fr, t_monster monster)
{
	t_vec3f			top_left;
	t_vec3f			bot_right;
	t_vec3f			monsterpos;
	...
	monsterpos = (t_vec3f) {monster.pos.x, sector->floorheight +
		d->monster_type[monster.id_type].floating +
			monster.height, monster.pos.y};
	(void)fr;
	scale = (t_vec2f){monster.width, 0};
	actualize_dir(-d->cam.rot, &scale);
	top_left = get_projected_vertex(d, (t_vec3f){monsterpos.x - scale.x,
			monsterpos.y, monsterpos.z - scale.y}, monsterpos);
	bot_right = get_projected_vertex(d, (t_vec3f){monsterpos.x + scale.x,
			monsterpos.y + d->monster_type[monster.id_type].floating -
			monster.height, monsterpos.z + scale.y}, monsterpos);
	if (top_left.z > 0.0 && bot_right.z > 0.0)
	{
		...
		a.ybot = &fr->ybottom[0];
		printf("%f\n", top_left.x);
		nb_of_anim = get_nb_anim_from_rot(monster.rot,
				monster.pos, vec3to2(d->cam.pos));
		if (nb_of_anim > 4)
		{
			nb_of_anim = 8 - nb_of_anim;
			display_sprite(a, d, d->monster_text[monster.id_type]
					[monster.anim_state][nb_of_anim], false);
		}
		else
			display_sprite(a, d, d->monster_text[monster.id_type]
					[monster.anim_state][nb_of_anim], true);
	}
}
...
void	draw_monster_test(t_data *d, t_frustum *fr, t_monster monster)
{
	...
	a.start.x = point_in_screen.x - (d->monster_text[monster.id_type]
		[monster.anim_state][nb_of_anim[0]]->w) / dist *
	...
	a.start.y = point_in_screen.y - (d->monster_text[monster.id_type]
		...
		dist * d->monster_type[monster.id_type].size;
	...
	if (nb_of_anim[1] < 4 && monster.anim_state < 10)
		display_sprite_one_point_rev(d, d->monster_text[monster.id_type]
				[monster.anim_state][nb_of_anim[0]], a);
	else
